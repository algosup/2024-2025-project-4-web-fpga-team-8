//Verilog generated by VPR 9.0.0-dev+v8.0.0-11943-g8cb20aa52-dirty from post-place-and-route implementation
module RisingEdge_DFlipFlop_AsyncResetHigh (
    input \D ,
    input \async_reset ,
    input \clk ,
    output \Q 
);

    //Wires
    wire \D_output_0_0 ;
    wire \async_reset_output_0_0 ;
    wire \clk_output_0_0 ;
    wire \latch_Q_output_0_0 ;
    wire \lut_gnd_output_0_0 ;
    wire \lut_$auto$rtlil.cc:2714:MuxGate$167_output_0_0 ;
    wire \lut_$auto$rtlil.cc:2714:MuxGate$169_output_0_0 ;
    wire \latch_$sdff~3^Q~0_output_0_0 ;
    wire \lut_$auto$rtlil.cc:2714:MuxGate$171_output_0_0 ;
    wire \latch_$sdff~1^Q~0_output_0_0 ;
    wire \lut_$auto$rtlil.cc:2714:MuxGate$173_output_0_0 ;
    wire \latch_$sdff~2^Q~0_output_0_0 ;
    wire \lut_$auto$rtlil.cc:2714:MuxGate$175_output_0_0 ;
    wire \latch_$sdff~0^Q~0_output_0_0 ;
    wire \lut_$auto$rtlil.cc:2714:MuxGate$175_input_0_3 ;
    wire \lut_$auto$rtlil.cc:2714:MuxGate$175_input_0_4 ;
    wire \lut_$auto$rtlil.cc:2714:MuxGate$173_input_0_3 ;
    wire \lut_$auto$rtlil.cc:2714:MuxGate$171_input_0_4 ;
    wire \lut_$auto$rtlil.cc:2714:MuxGate$169_input_0_1 ;
    wire \lut_$auto$rtlil.cc:2714:MuxGate$167_input_0_0 ;
    wire \latch_$sdff~0^Q~0_clock_0_0 ;
    wire \latch_$sdff~2^Q~0_clock_0_0 ;
    wire \latch_$sdff~1^Q~0_clock_0_0 ;
    wire \latch_$sdff~3^Q~0_clock_0_0 ;
    wire \latch_Q_clock_0_0 ;
    wire \Q_input_0_0 ;
    wire \lut_$auto$rtlil.cc:2714:MuxGate$175_input_0_1 ;
    wire \lut_$auto$rtlil.cc:2714:MuxGate$173_input_0_1 ;
    wire \lut_$auto$rtlil.cc:2714:MuxGate$171_input_0_1 ;
    wire \lut_$auto$rtlil.cc:2714:MuxGate$169_input_0_2 ;
    wire \lut_$auto$rtlil.cc:2714:MuxGate$167_input_0_2 ;
    wire \latch_Q_input_0_0 ;
    wire \latch_$sdff~3^Q~0_input_0_0 ;
    wire \lut_$auto$rtlil.cc:2714:MuxGate$167_input_0_1 ;
    wire \latch_$sdff~1^Q~0_input_0_0 ;
    wire \lut_$auto$rtlil.cc:2714:MuxGate$173_input_0_0 ;
    wire \latch_$sdff~2^Q~0_input_0_0 ;
    wire \lut_$auto$rtlil.cc:2714:MuxGate$169_input_0_3 ;
    wire \latch_$sdff~0^Q~0_input_0_0 ;
    wire \lut_$auto$rtlil.cc:2714:MuxGate$171_input_0_2 ;

    //IO assignments
    assign \Q  = \Q_input_0_0 ;
    assign \D_output_0_0  = \D ;
    assign \async_reset_output_0_0  = \async_reset ;
    assign \clk_output_0_0  = \clk ;

    //Interconnect
    fpga_interconnect \routing_segment_D_output_0_0_to_lut_$auto$rtlil.cc:2714:MuxGate$175_input_0_3  (
        .datain(\D_output_0_0 ),
        .dataout(\lut_$auto$rtlil.cc:2714:MuxGate$175_input_0_3 )
    );

    fpga_interconnect \routing_segment_async_reset_output_0_0_to_lut_$auto$rtlil.cc:2714:MuxGate$175_input_0_4  (
        .datain(\async_reset_output_0_0 ),
        .dataout(\lut_$auto$rtlil.cc:2714:MuxGate$175_input_0_4 )
    );

    fpga_interconnect \routing_segment_async_reset_output_0_0_to_lut_$auto$rtlil.cc:2714:MuxGate$173_input_0_3  (
        .datain(\async_reset_output_0_0 ),
        .dataout(\lut_$auto$rtlil.cc:2714:MuxGate$173_input_0_3 )
    );

    fpga_interconnect \routing_segment_async_reset_output_0_0_to_lut_$auto$rtlil.cc:2714:MuxGate$171_input_0_4  (
        .datain(\async_reset_output_0_0 ),
        .dataout(\lut_$auto$rtlil.cc:2714:MuxGate$171_input_0_4 )
    );

    fpga_interconnect \routing_segment_async_reset_output_0_0_to_lut_$auto$rtlil.cc:2714:MuxGate$169_input_0_1  (
        .datain(\async_reset_output_0_0 ),
        .dataout(\lut_$auto$rtlil.cc:2714:MuxGate$169_input_0_1 )
    );

    fpga_interconnect \routing_segment_async_reset_output_0_0_to_lut_$auto$rtlil.cc:2714:MuxGate$167_input_0_0  (
        .datain(\async_reset_output_0_0 ),
        .dataout(\lut_$auto$rtlil.cc:2714:MuxGate$167_input_0_0 )
    );

    fpga_interconnect \routing_segment_clk_output_0_0_to_latch_$sdff~0^Q~0_clock_0_0  (
        .datain(\clk_output_0_0 ),
        .dataout(\latch_$sdff~0^Q~0_clock_0_0 )
    );

    fpga_interconnect \routing_segment_clk_output_0_0_to_latch_$sdff~2^Q~0_clock_0_0  (
        .datain(\clk_output_0_0 ),
        .dataout(\latch_$sdff~2^Q~0_clock_0_0 )
    );

    fpga_interconnect \routing_segment_clk_output_0_0_to_latch_$sdff~1^Q~0_clock_0_0  (
        .datain(\clk_output_0_0 ),
        .dataout(\latch_$sdff~1^Q~0_clock_0_0 )
    );

    fpga_interconnect \routing_segment_clk_output_0_0_to_latch_$sdff~3^Q~0_clock_0_0  (
        .datain(\clk_output_0_0 ),
        .dataout(\latch_$sdff~3^Q~0_clock_0_0 )
    );

    fpga_interconnect \routing_segment_clk_output_0_0_to_latch_Q_clock_0_0  (
        .datain(\clk_output_0_0 ),
        .dataout(\latch_Q_clock_0_0 )
    );

    fpga_interconnect \routing_segment_latch_Q_output_0_0_to_Q_input_0_0  (
        .datain(\latch_Q_output_0_0 ),
        .dataout(\Q_input_0_0 )
    );

    fpga_interconnect \routing_segment_lut_gnd_output_0_0_to_lut_$auto$rtlil.cc:2714:MuxGate$175_input_0_1  (
        .datain(\lut_gnd_output_0_0 ),
        .dataout(\lut_$auto$rtlil.cc:2714:MuxGate$175_input_0_1 )
    );

    fpga_interconnect \routing_segment_lut_gnd_output_0_0_to_lut_$auto$rtlil.cc:2714:MuxGate$173_input_0_1  (
        .datain(\lut_gnd_output_0_0 ),
        .dataout(\lut_$auto$rtlil.cc:2714:MuxGate$173_input_0_1 )
    );

    fpga_interconnect \routing_segment_lut_gnd_output_0_0_to_lut_$auto$rtlil.cc:2714:MuxGate$171_input_0_1  (
        .datain(\lut_gnd_output_0_0 ),
        .dataout(\lut_$auto$rtlil.cc:2714:MuxGate$171_input_0_1 )
    );

    fpga_interconnect \routing_segment_lut_gnd_output_0_0_to_lut_$auto$rtlil.cc:2714:MuxGate$169_input_0_2  (
        .datain(\lut_gnd_output_0_0 ),
        .dataout(\lut_$auto$rtlil.cc:2714:MuxGate$169_input_0_2 )
    );

    fpga_interconnect \routing_segment_lut_gnd_output_0_0_to_lut_$auto$rtlil.cc:2714:MuxGate$167_input_0_2  (
        .datain(\lut_gnd_output_0_0 ),
        .dataout(\lut_$auto$rtlil.cc:2714:MuxGate$167_input_0_2 )
    );

    fpga_interconnect \routing_segment_lut_$auto$rtlil.cc:2714:MuxGate$167_output_0_0_to_latch_Q_input_0_0  (
        .datain(\lut_$auto$rtlil.cc:2714:MuxGate$167_output_0_0 ),
        .dataout(\latch_Q_input_0_0 )
    );

    fpga_interconnect \routing_segment_lut_$auto$rtlil.cc:2714:MuxGate$169_output_0_0_to_latch_$sdff~3^Q~0_input_0_0  (
        .datain(\lut_$auto$rtlil.cc:2714:MuxGate$169_output_0_0 ),
        .dataout(\latch_$sdff~3^Q~0_input_0_0 )
    );

    fpga_interconnect \routing_segment_latch_$sdff~3^Q~0_output_0_0_to_lut_$auto$rtlil.cc:2714:MuxGate$167_input_0_1  (
        .datain(\latch_$sdff~3^Q~0_output_0_0 ),
        .dataout(\lut_$auto$rtlil.cc:2714:MuxGate$167_input_0_1 )
    );

    fpga_interconnect \routing_segment_lut_$auto$rtlil.cc:2714:MuxGate$171_output_0_0_to_latch_$sdff~1^Q~0_input_0_0  (
        .datain(\lut_$auto$rtlil.cc:2714:MuxGate$171_output_0_0 ),
        .dataout(\latch_$sdff~1^Q~0_input_0_0 )
    );

    fpga_interconnect \routing_segment_latch_$sdff~1^Q~0_output_0_0_to_lut_$auto$rtlil.cc:2714:MuxGate$173_input_0_0  (
        .datain(\latch_$sdff~1^Q~0_output_0_0 ),
        .dataout(\lut_$auto$rtlil.cc:2714:MuxGate$173_input_0_0 )
    );

    fpga_interconnect \routing_segment_lut_$auto$rtlil.cc:2714:MuxGate$173_output_0_0_to_latch_$sdff~2^Q~0_input_0_0  (
        .datain(\lut_$auto$rtlil.cc:2714:MuxGate$173_output_0_0 ),
        .dataout(\latch_$sdff~2^Q~0_input_0_0 )
    );

    fpga_interconnect \routing_segment_latch_$sdff~2^Q~0_output_0_0_to_lut_$auto$rtlil.cc:2714:MuxGate$169_input_0_3  (
        .datain(\latch_$sdff~2^Q~0_output_0_0 ),
        .dataout(\lut_$auto$rtlil.cc:2714:MuxGate$169_input_0_3 )
    );

    fpga_interconnect \routing_segment_lut_$auto$rtlil.cc:2714:MuxGate$175_output_0_0_to_latch_$sdff~0^Q~0_input_0_0  (
        .datain(\lut_$auto$rtlil.cc:2714:MuxGate$175_output_0_0 ),
        .dataout(\latch_$sdff~0^Q~0_input_0_0 )
    );

    fpga_interconnect \routing_segment_latch_$sdff~0^Q~0_output_0_0_to_lut_$auto$rtlil.cc:2714:MuxGate$171_input_0_2  (
        .datain(\latch_$sdff~0^Q~0_output_0_0 ),
        .dataout(\lut_$auto$rtlil.cc:2714:MuxGate$171_input_0_2 )
    );


    //Cell instances
    LUT_K #(
        .K(5),
        .LUT_MASK(32'b00000100000001000000010100000000)
    ) \lut_$auto$rtlil.cc:2714:MuxGate$175  (
        .in({
            \lut_$auto$rtlil.cc:2714:MuxGate$175_input_0_4 ,
            \lut_$auto$rtlil.cc:2714:MuxGate$175_input_0_3 ,
            1'b0,
            \lut_$auto$rtlil.cc:2714:MuxGate$175_input_0_1 ,
            1'b0
         }),
        .out(\lut_$auto$rtlil.cc:2714:MuxGate$175_output_0_0 )
    );

    DFF #(
        .INITIAL_VALUE(1'b0)
    ) \latch_$sdff~0^Q~0  (
        .D(\latch_$sdff~0^Q~0_input_0_0 ), 
        .Q(\latch_$sdff~0^Q~0_output_0_0 ), 
        .clock(\latch_$sdff~0^Q~0_clock_0_0 )
    );
    LUT_K #(
        .K(5),
        .LUT_MASK(32'b00000000000000000000000000000000)
    ) \lut_gnd  (
        .in({
            1'b0,
            1'b0,
            1'b0,
            1'b0,
            1'b0
         }),
        .out(\lut_gnd_output_0_0 )
    );

    LUT_K #(
        .K(5),
        .LUT_MASK(32'b00000000000000000000110000001010)
    ) \lut_$auto$rtlil.cc:2714:MuxGate$173  (
        .in({
            1'b0,
            \lut_$auto$rtlil.cc:2714:MuxGate$173_input_0_3 ,
            1'b0,
            \lut_$auto$rtlil.cc:2714:MuxGate$173_input_0_1 ,
            \lut_$auto$rtlil.cc:2714:MuxGate$173_input_0_0 
         }),
        .out(\lut_$auto$rtlil.cc:2714:MuxGate$173_output_0_0 )
    );

    DFF #(
        .INITIAL_VALUE(1'b0)
    ) \latch_$sdff~2^Q~0  (
        .D(\latch_$sdff~2^Q~0_input_0_0 ), 
        .Q(\latch_$sdff~2^Q~0_output_0_0 ), 
        .clock(\latch_$sdff~2^Q~0_clock_0_0 )
    );
    LUT_K #(
        .K(5),
        .LUT_MASK(32'b00000000010001000000000001010000)
    ) \lut_$auto$rtlil.cc:2714:MuxGate$171  (
        .in({
            \lut_$auto$rtlil.cc:2714:MuxGate$171_input_0_4 ,
            1'b0,
            \lut_$auto$rtlil.cc:2714:MuxGate$171_input_0_2 ,
            \lut_$auto$rtlil.cc:2714:MuxGate$171_input_0_1 ,
            1'b0
         }),
        .out(\lut_$auto$rtlil.cc:2714:MuxGate$171_output_0_0 )
    );

    DFF #(
        .INITIAL_VALUE(1'b0)
    ) \latch_$sdff~1^Q~0  (
        .D(\latch_$sdff~1^Q~0_input_0_0 ), 
        .Q(\latch_$sdff~1^Q~0_output_0_0 ), 
        .clock(\latch_$sdff~1^Q~0_clock_0_0 )
    );
    LUT_K #(
        .K(5),
        .LUT_MASK(32'b00000000000000000101000101000000)
    ) \lut_$auto$rtlil.cc:2714:MuxGate$169  (
        .in({
            1'b0,
            \lut_$auto$rtlil.cc:2714:MuxGate$169_input_0_3 ,
            \lut_$auto$rtlil.cc:2714:MuxGate$169_input_0_2 ,
            \lut_$auto$rtlil.cc:2714:MuxGate$169_input_0_1 ,
            1'b0
         }),
        .out(\lut_$auto$rtlil.cc:2714:MuxGate$169_output_0_0 )
    );

    DFF #(
        .INITIAL_VALUE(1'b0)
    ) \latch_$sdff~3^Q~0  (
        .D(\latch_$sdff~3^Q~0_input_0_0 ), 
        .Q(\latch_$sdff~3^Q~0_output_0_0 ), 
        .clock(\latch_$sdff~3^Q~0_clock_0_0 )
    );
    LUT_K #(
        .K(5),
        .LUT_MASK(32'b00000000000000000000000011100100)
    ) \lut_$auto$rtlil.cc:2714:MuxGate$167  (
        .in({
            1'b0,
            1'b0,
            \lut_$auto$rtlil.cc:2714:MuxGate$167_input_0_2 ,
            \lut_$auto$rtlil.cc:2714:MuxGate$167_input_0_1 ,
            \lut_$auto$rtlil.cc:2714:MuxGate$167_input_0_0 
         }),
        .out(\lut_$auto$rtlil.cc:2714:MuxGate$167_output_0_0 )
    );

    DFF #(
        .INITIAL_VALUE(1'b0)
    ) \latch_Q  (
        .D(\latch_Q_input_0_0 ), 
        .Q(\latch_Q_output_0_0 ), 
        .clock(\latch_Q_clock_0_0 )
    );

endmodule
